...
struct info_littab
{
    string address,length, value,block;
    int num;
    char exist;
    info_littab()
    {
...
typedef std::map<label,info_littab>   MapType5;
...
//THis maps the Littab with the value
MapType5 LITTAB;
...
string find_littab(int num)
{
     for (MapType5::const_iterator it = LITTAB.begin();;++it)
         if(it->second.num==num)
            return it->first;
}
